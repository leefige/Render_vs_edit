...
Color PathTracing::tracing(Light light, int depth, vector<Object *> &objs) {
    ...
    float3 nl = inside == 1 ? n_vec * (-1) : n_vec;
    ...
    if (ref_tp == SPECULAR)
    {
        return emit + color.mix(tracing(Light(pos, light.direct - light.direct * n_vec * n_vec * 2), depth, objs));
    }
    ...
    {
        Light reflRay = Light(pos, light.direct - light.direct * (n_vec * n_vec) * 2);
        ...
        double r0 = a * a / b / b;
        double c = 1 - (inside ? tdir*n_vec : -ddn);
